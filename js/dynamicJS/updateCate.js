...
Vue.component('v-contaddcate', {
	...
	methods: {
		getCateDetail:function(){
			...
			this.cateDetailId = cateItem.cateDetailId;
			this.cateId = cateItem.cateId;
		...
		submit: function() {
			...
			var paramData = {
				...
				"cateDetailId": this.cateDetailId,
				"cateId": this.cateId,
				"markCate": "update_cate"
			};
			console.log("上送数据--------->");
			...
			$.ajax({
				...
				success: function(data) {　　　　　　　　　　　　 //var message = $.parseJSON(data);//后台返回的json数据需要转为对象
					...
					} else if (data.rcode === "408006") {
						vm.Alert("修改失败", "warning", 2000);
						//alert("该分类已存在..");
					}
				},
				error: function() {
